<template>
  <div>
    <div class="detail" ref="detail" v-if="!flag">
      ...
      <div class="info" v-if="Object.keys(carObj).length">
        ...
        <button class="info-btn" @click="floorPrice(carObj.list[0].car_name)">{{carObj.BottomEntranceTitle}}</button>
      </div>
      <div class="container">
        <div class="con-div">
          <span
        ...
        <div class="con-con">
          <div v-for="(item, index) in dataObj[item]" :key="index" class="item">
            ...
            <div class="con-con-div" v-for="(item2, i2) in item.list" :key="i2">
              ...
              <p>
                <span v-for="(item3, i3) in item2.key3" :key="i3">{{item3}}</span>
              </p>
              <button
                class="con-con-btn"
                @click="floorPrice(item2.key1)"
              >{{carObj.BottomEntranceTitle}}</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="flag">
      <Price  :carObj = 'this.carObj' :title = 'this.title'/>
    </div>
  </div>
</template>
